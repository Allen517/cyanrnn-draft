...
event information which may be ``forgetten'' by sequential transitions in RNN,
called crossing dependency problems in CDM.
...
help to efficiently utilize the event embeddings in attention mechanism. Next
we will exploit the answers how the coverage can help to boost predictive
...
the weight $\alpha_{k,i}$ related to $i$-th event embedding at each step $k$, in
grayscale (0:black, 1:white). (a) the alignments inferred by CYAN-RNN; (b) the
...
\noindent{\textbf{Alignment quality.}} 
Firstly, we expect to check if coverage can reduce the over-dependent and
...
in each plot represents the alignment weights associated with event embeddings.
...
concentrate more on unused event embeddings, which is consistent to well-studied
phenomonen in published works~\cite{}. Moreover, we wonder if the
